  ...
  return (
    <div className="flex flex-col bg-[#0a0a0b] p-8 min-h-full">
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          <div className="text-[10px] text-zinc-600 uppercase tracking-widest">
            Reader Belief State: <span className={`font-bold ml-2 ${hasWound ? 'text-crimson animate-pulse' : 'text-zinc-800'}`}>
              {hasWound ? 'DETECTING DRIFT' : 'STANDBY'}
            </span>
          </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 gap-12 flex-1 min-h-0">
        ...
        <section className={`bg-zinc-900/10 border border-zinc-900 rounded-sm p-8 h-72 flex flex-col relative overflow-hidden group transition-opacity duration-1000 ${hasWound ? 'opacity-100' : 'opacity-20'}`}>
        ...
        {/* Story Spine */}
        <section className={`transition-opacity duration-1000 ${hasWound ? 'opacity-100' : 'opacity-20'}`}>
          ...
          <div className="flex h-32 w-full bg-zinc-900/40 border border-zinc-800 rounded divide-x divide-zinc-900 overflow-hidden relative">
            {!hasWound && (
              <div className="absolute inset-0 flex items-center justify-center z-10">
                <div className="text-[9px] text-zinc-800 font-mono uppercase tracking-widest">Heatmap Offline</div>
              </div>
            )}
            {[1, 2, 3, 4, 5, 6, 7, 8].map(i => (
              <div key={i} className="flex-1 flex flex-col p-3 group cursor-pointer hover:bg-zinc-800 transition-all duration-300">
                <div className="flex justify-between items-center mb-3">
                  ...
                  <div className={`w-1 h-1 rounded-full ${hasWound && Math.random() > 0.7 ? 'bg-crimson shadow-[0_0_4px_#990000]' : 'bg-zinc-800'}`} />
                </div>
                {hasWound && (
        ...
        <div className="grid grid-cols-2 gap-8">
          <section className={`bg-zinc-900/10 border border-zinc-900 rounded-sm p-6 h-64 flex flex-col relative overflow-hidden transition-opacity duration-1000 ${hasWound ? 'opacity-100' : 'opacity-20'}`}>
            ...
            <div className="flex-1 flex items-center justify-center relative">
              {hasWound ? (
                ...
                <div className="text-[9px] text-zinc-800 font-mono uppercase">Nodes Undefined</div>
              )}
            </div>
          </section>

          <section className={`bg-zinc-900/10 border border-zinc-900 rounded-sm p-6 h-64 flex flex-col transition-opacity duration-1000 ${hasWound ? 'opacity-100' : 'opacity-20'}`}>
             <div className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold mb-4">
            ...
            <div className="space-y-3 overflow-y-auto flex-1">
              {hasWound ? (
                ...
                <div className="h-full flex items-center justify-center">
                  <div className="text-[9px] text-zinc-800 font-mono uppercase">Feed Inactive</div>
                </div>
              )}
            </div>
          </section>
